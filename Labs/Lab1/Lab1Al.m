%________Initialization________%
clc
clear all
close all
...
for a = 1:test_size
    ...
    if result(a) == Lte28(a)
        ...
        error = error + 1;
        fprintf('The training set failed to identify element %d, which is the number: %d, but was %d\n',a, Lte28(a),result(a));

        %Print Images
        subplot(21,21,error);
        Shape=reshape(Te28(:,a),28,28);
        imshow(Shape)
        title([result(a)])
    end
